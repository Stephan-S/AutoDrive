			...
			if serverName is not None:
				...
				while not connected:
					try:
						...
						msg = "Register|%s/%s/%s/%s" % (self.userName, self.hub, self.hubPasswordIn, self.hubPasswordOut)
						outSock.send(msg)
					...
					if messageType == "Update":
						...
						for commodity in commidityList:
							...
							if amount != amountLocal and boughtLocal == 0 and soldLocal == 0:
								...
								changed = True
							debugPrintMessage = "%s\n\t%s: amount: %s price: %s" % (debugPrintMessage, name.ljust(25), ("%s" % amount).ljust(10),("%s" % price).ljust(6))
						...
						if messageType == "Ack" and message == "Ok":
							...
							self.root[0].find(".//*[name='%s']" % self.updateInfo["commodity"]).find('bought').text = "0"
		...
		while True:
			...
			if allowed:
				...
				for commodity in commidityList:
					...
					if boughtLocal > 0 or soldLocal > 0 and self.connection[0] != "null" and not self.updateInfo["send"]:
						messageType = "Update"
